...
def load_data():
    ...
    # Rename delivereddate → delivery (semantic rename)
    deliv.rename(columns={"delivereddate": "delivery"}, inplace=True)

    # Filter for 2025
    prod = prod[prod["eventdate"].dt.year == 2025]
    deliv = deliv[deliv["delivery"].dt.year == 2025]
...
view_mode = st.sidebar.radio("View Mode", ["YTD", "Monthly"])
...
if view_mode == "Monthly":
    st.line_chart(
        monthly_perf.set_index("month")[
            ["producedontime_prod", "delivered_on-time_deliv"]
        ]
    )
else:
    ytd_df = pd.DataFrame(
